...
function minimumWindowSubstring(S, T) {
  ...
  while (end < S.length) {
    ...
    if (currentChar in targetChars) {
      if (targetChars[currentChar] === 0) { charsRemaining -= 1; }
      targetChars[currentChar] += 1
    }
    ...
    while (charsRemaining === 0) {
      ...
      if (S[start] in targetChars) {
        targetChars[S[start]] -= 1;
        if (targetChars[S[start]] === 0) {
          charsRemaining += 1;
        }
      }
      start += 1;
